<template>
  <div id="containerNetworkEchars" :style="{width: '100%', height: '300px'}"></div>
</template>

<script>
// 引入基本模板
import echarts from 'echarts'// 引入柱状图组件
...
export default {
  ...
  methods: {
    async getContainerNetwork (host, dashboardTime) {
      this.x_coordinate = []
      this.series = []
      this.legend = []

      const { data: res } = await this.$http.post('monitor/container/network', { 'field': ['rx_bytes'], 'dashboard_time': dashboardTime, 'host_name': host })
      ...
      for (var v = 0; v < res.data.length; v++) {
        ...
        for (var n = 0; n < tmpLegend.length; n++) {
          this.legend.push(tmpLegend[n] + '[container_name=' + res.data[v].tags.container_name + ']')
        }
        ...
        for (var m = 0; m < tmpLegend.length; m++) {
          tmpSeries.push({
            name: tmpLegend[m] + '[container_name=' + res.data[v].tags.container_name + ']',
            type: 'line',
            data: []
          })
        }
    ...
    drawLine () {
      ...
      let myChart = echarts.init(document.getElementById('containerNetworkEchars'))
      // 绘制图表
      myChart.clear()
      myChart.setOption({
        title: {
