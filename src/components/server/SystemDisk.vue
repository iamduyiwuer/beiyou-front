<template>
  <div id="systemDiskEchars" :style="{width: '100%', height: '300px'}"></div>
</template>
...
export default {
  ...
  methods: {
    async getSystemDisk (host, dashboardTime) {
      this.x_coordinate = []
      this.series = []
      this.legend = []

      const { data: res } = await this.$http.post('monitor/system/disk', { 'field': ['used_percent'], 'dashboard_time': dashboardTime, 'host_name': host })
      if (res.meta.status !== 200) return this.$message.error('获取服务器硬盘数据失败')
    ...
    drawLine () {
      ...
      let myChart = echarts.init(document.getElementById('systemDiskEchars'))
      // 绘制图表
      myChart.clear()
      myChart.setOption({
        title: {
